"""Backfill historical capability identities; dry-run unless --apply is given."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="补齐已发布历史教材的稳定能力身份并重放学习画像",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    try:
        with sessions() as db:
            summary = repair_published_historical_capability_identities(db)
            if arguments.apply:
                db.commit()
            else:
                db.rollback()
