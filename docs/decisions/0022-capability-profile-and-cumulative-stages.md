...
- `review_task_plan_v1` 在 Review Assignment 创建时冻结两类不同计划：遗忘到期只按用户已取得的当前阶段安排同层再激活任务，证据效力仅为 activation；有意识强化才读取首个缺失 Stage Criterion，并且只有路线中已发布真实验证机会时才给出下一阶任务。白银复习因此规划机制/边界口试，黄金规划标准应用，钻石规划陌生迁移；近期错题补强仍可先使用青铜诊断，但不能降低历史阶段或制造升段；
- 白银口试、黄金应用和钻石迁移的再激活执行器已经接入：每次 Review Assignment 独立冻结新任务版本、原始提交和独立评定事实；任务必须完整覆盖当前阶段量规，黄金不得复用正文案例，钻石必须保留陌生情境和能力子网知识重组。到期窗口、无辅助、题面独立以及作者/评定者模型家族分离共同决定再激活资格；
- `capability_review` 证据明确只拥有 activation 效力：无论任务处于白银、黄金还是钻石，其 `capability` 晋级资格均为 ineligible，也不改写概念掌握度或旧段位；合格通过只刷新能力复习日程和当前可调用状态，失败则进入能力再激活补强，不降低历史最高阶段；
- 复习工作台已经按阶段执行青铜选择、白银口试、黄金应用和钻石迁移再激活，并显式展示“再激活不升段”；再激活合格后，统一强化入口只读取 Assignment 创建时冻结的下一阶计划：白银进入原课程 Ask Me，黄金和钻石在工作台完成对应的正式已发布任务，最终仍由原有正式评定与投影链决定是否晋级；
- 节末结算已切换为 `capability_settlement_v1`：只结算稳定 Capability Revision 的四级阶段、独立证据成熟度、当前可用状态与下一项量规，不再把概念节点的旧六级段位作为用户结算结果；
- 个人知识地图已切换为 `personal_capability_map_v2`：图中一级节点是路线内稳定能力，详情展示该能力冻结的 anchor / required / supporting 知识节点及正式关系；能力之间只在共享精确知识身份时连线，因此不会再为每个零散概念各发一枚段位；系列筛选后的覆盖率也由已取得正式阶段的能力数计算；
- `CapabilityStateProjection v1` 从不可变事实重建能力阶段、证据成熟度和激活状态；
- 累计投影要求逐级满足，选择题即使沿用旧体系的 `transfer` 维度，也只能满足所绑定的青铜标准；
- 旧六级知识节点投影仅在内部兼容链路并行重建，正文生成上下文、复习、节末结算和知识地图均优先或唯一消费新能力状态；
- 数据库迁移、跨书能力复用、禁止跳级、旧钻石/新青铜双轨差异和真实答题链路已有自动化测试。

仍未完成的范围：

- 历史旧证据的显式解析与 unresolved 队列；
- 旧六级投影停止内部兼容写入。目前它不再驱动新节末结算和知识地图，但仍为旧审计、旧笔记注释与未迁移数据并行重建；
- 书架上的阅读完成百分比仍只表达教材进度，不等同于能力覆盖；系列能力覆盖已进入知识地图，是否还要在书架重复展示属于后续产品表达选择，而不是画像链路阻塞。

因此当前新路径可以称为“知识与能力身份、青铜至钻石正式证据链路、基于画像的复习与主动强化，以及主要用户可见画像读取已经闭环”。剩余工作只涉及明确不投入本阶段的历史证据迁移，以及逐步删除内部旧六级兼容投影；它们不再阻塞新学习路径。
